...
def mostrar_informe_env_sol():
    ...
    if not os.path.exists(file_path_Env):
        ...
        df.rename(columns={'Mes:':'Mes', 'Mes_N': 'IdMes', 'Año:':'Año', 'Máquina / Equipo:':'Máquina / Equipo', 'Turno:':'Turno', 'Unidades Producidas (Conformes) :':'Unidades Producidas',
        ...
        #Crear tabla pivote incluyendo Codigo y Máquina y IdMes el ultimo para ordenar
        ...
        #Crear tabla pivote agrupando por Año y mes, inclutendo IdMes para ordenar
        ...
        df_Env_MES_visual = df_Env_MES_visual.sort_values(by=['Año', 'IdMes'], ascending=[True, True])
        ...
        if df_Env_MES.empty:
            ...
            with col_izq:
                ...
                st.markdown(f'<div class="metric-card">Total Teórico [kg]<br><b>{df_Env_MES_visual["Cantidad Teórica a Empacar [kg]"].sum():,.0f}</b></div>', unsafe_allow_html=True)
                st.markdown(f'<div class="metric-card">Total Real [kg]<br><b>{df_Env_MES_visual["Cantidad Real Empacada [kg]"].sum():,.0f}</b></div>', unsafe_allow_html=True)
            ...
            with col_der:
                ...
                st.markdown("""
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <h1 style="margin: 0; color: black; font-size: 16px;">Informe Mensual de Sobrepeso - Envase Soluble</h1>
                ...
                excel_data = fc.convertir_a_excel(df_Env_MES_visual)
            ...
            ## ==============================================================================
            ## Agregar Sobrepeso segun Máquina(diagrama de barras horizontal)
            ## ==============================================================================
            ...
            if 'Sobrepeso [%]' in df_Env_MES.columns:
                st.markdown(
                    ...
                    """, unsafe_allow_html=True)
                #Eje x
                histograma = px.histogram(df_Env_MES, x='Sobrepeso [%]', nbins=25)
                #Eje y
            ...
            df_Env_Dia = df_filtrado.pivot_table(
            ...
            aggfunc={'Unidades Producidas': 'sum',
                     ...
                     'Peso Promedio de la unidad (K):':'mean',
                     'Cantidad Real Empacada [kg]':'sum',
                     'Cantidad Teórica a Empacar [kg]':'sum','Diferencia [kg]':'sum'
                    }
            ).reset_index()   

            df_Env_Dia['Sobrepeso [%]'] = (df_Env_Dia['Diferencia [kg]'] / df_Env_Dia['Cantidad Teórica a Empacar [kg]']) * 100
            ...
            st.markdown(
                ...
                """, unsafe_allow_html=True)
            if not df_Env_Dia.empty:
                fig = px.line(
                    df_Env_Dia.sort_values('Fecha'),
                    x='Fecha',
                    y='Sobrepeso [%]',
